...
/// Detects king mate: checkmate where the king delivers the final check
/// (king moves to give discovered checkmate).
pub struct KingMateAnalyzer {
    username: String,
...
impl GameAnalyzer for KingMateAnalyzer {
    ...
    fn process_move(&mut self, ctx: &MoveContext) {
        ...
        // Must be a king move (normal move, not castling for this analyzer)
        match ctx.mv {
            Move::Normal { role: Role::King, .. } => {}
            _ => return,
        }
