"""
Unified analyzer for finding rook sacrifices.
"""
import chess
...
class UnifiedRookSacrificeAnalyzer(UnifiedAnalyzerBase):
    """
    Finds games where the user sacrificed a rook.
    """
    ...
    def start_game(self, game_data: GameData, user_is_white: bool, user_is_black: bool):
        ...
        self.our_rook_was_recaptured = False
    ...
    def process_move(self, context: MoveContext):
        ...
        if context.is_user_move and context.board.is_capture(context.move):
            ...
            if moving_piece and moving_piece.piece_type == chess.ROOK:
                ...
                # Exclude if capturing opponent's rook (that's a trade)
                if captured_piece and captured_piece.piece_type == chess.ROOK:
                    return

                # Exclude if capturing opponent's queen (that's just winning material)
                ...
                rook_pinned = context.board.is_pinned(user_color, context.move.from_square)
        ...
        if context.is_opponent_move and self.potential_sacrifice:
            ...
            if context.move_number == self.potential_sacrifice['sacrifice_move_number'] + 1:
                if context.move.to_square == captured_square:
                    self.our_rook_was_recaptured = True
        ...
        # Check user's move after rook was recaptured
        if context.is_user_move and self.our_rook_was_recaptured and self.recapture_move_number:
            if context.move_number == self.recapture_move_number + 1:
                if context.board.is_capture(context.move):
                    ...
                    if captured_piece and captured_piece.piece_type == chess.ROOK:
                        # User took opponent's rook - NOT a sacrifice
                        self.potential_sacrifice = None
                        self.our_rook_was_recaptured = False
                        self.recapture_move_number = None
                        return

                # User didn't take opponent's rook - IS a sacrifice!
    ...
    def _record_sacrifice(self, context: MoveContext):
        ...
        if self.is_checkmate_win and self.game_data.moves:
            ...
            is_checkmate_in_range = 2 <= moves_after_sacrifice <= 5
    ...
    def get_final_results(self) -> List[Dict[str, Any]]:
        ...
        for ref in self.all_sacrifice_refs:
            ...
            replay_data = build_replay_data(
                ...
                feature_name="rook_sacrifice"
            )

            finding = {
                "feature_name": "rook_sacrifice",
                "display_name": "Rook Sacrifice",
                "game_metadata": {
