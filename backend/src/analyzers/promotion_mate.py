"""
Unified analyzer for finding promotion mates - checkmate delivered by promoting a pawn when down material.
Uses the unified move-by-move approach for efficiency.
"""
import chess
import re
from typing import List, Dict, Any, Optional
from ..unified_analyzer import UnifiedAnalyzerBase, MoveContext
from ..game_data import GameData


class UnifiedPromotionMateAnalyzer(UnifiedAnalyzerBase):
    """
    Unified version of PromotionMateAnalyzer.
    Finds games where the user delivered checkmate with a pawn promotion while having less material.
    Uses unified move-by-move processing for better performance.
    """
    
    PIECE_VALUES = {'q': 9, 'r': 5, 'b': 3, 'n': 3, 'p': 1}
    
    def __init__(self, username: str):
        """Initialize with the username to filter for."""
        super().__init__(username)
        self.all_findings = []  # Store all findings across games (for get_final_results)
    
    def start_game(self, game_data: GameData, user_is_white: bool, user_is_black: bool):
        """
        Initialize analyzer state for a new game.
        
        Args:
            game_data: The game data
            user_is_white: Whether user is playing white
            user_is_black: Whether user is playing black
        """
        super().start_game(game_data, user_is_white, user_is_black)
        
        # Quick filter: must end in checkmate
        self.has_checkmate = '#' in self.game_data.pgn
        
        # Track state
        self.found_promotion_mate = False
        self.material_deficit = 0
    
    def process_move(self, context: MoveContext):
        """Process a single move. No action needed - we check in finish_game()."""
        pass
    
    def finish_game(self) -> List[Dict[str, Any]]:
        """
        Finalize analysis for the game and return findings.
        Called after all moves have been processed.
        
        Checks if the last move was a promotion checkmate.
        Matches simplified logic: check move string pattern for promotion + mate.
        
        Returns:
            List of findings for this game
        """
        # Quick filters
        if not self.has_checkmate:
            return []
        
        # Check if user won
        result = self.game_data.metadata.result
        user_won = (result == "1-0" and self.user_is_white) or (result == "0-1" and self.user_is_black)
        if not user_won:
            return []
        
        # Check moves list (more reliable than last_move_context if PGN parsing fails)
        moves = self.game_data.moves
        if not moves:
            return []
        
        last_move = moves[-1]
        
        # Promotion mate: contains '=' (promotion) and '#' (checkmate)
        # Examples: e8=Q#, exd8=N#, h8=R#
        if '=' not in last_move or '#' not in last_move:
            return []
        
        # Verify the last move was made by the user
        # If user is white, last move should be on odd move number (white moves)
        # If user is black, last move should be on even move number (black moves)
        # Move numbers: 1=white, 2=black, 3=white, 4=black, etc.
        last_move_number = len(moves)
        if self.user_is_white:
            # White moves on odd numbers (1, 3, 5, ...)
            if last_move_number % 2 == 0:
                return []  # Last move was black's, not user's
        else:  # user_is_black
            # Black moves on even numbers (2, 4, 6, ...)
            if last_move_number % 2 == 1:
                return []  # Last move was white's, not user's
        
        # Extract promoted piece
        promoted_piece = self._get_promoted_piece(last_move)
        promoted_value = self.PIECE_VALUES.get(promoted_piece.lower()[0] if promoted_piece else 'q', 0)
        
        # Check material disadvantage NOW (needed for get_game_points())
        # Extract FEN and check material
        fen = self._extract_fen(self.game_data.pgn)
        if fen:
            white_material, black_material = self._material_from_fen(fen)
            
            # Subtract promoted piece (it's in the FEN but we want "before" material)
            if self.user_is_white:
                winner_material = white_material - promoted_value
                loser_material = black_material
            else:
                winner_material = black_material - promoted_value
                loser_material = white_material
            
            # Only count if winner had less material
            if winner_material < loser_material:
                self.found_promotion_mate = True
                self.material_deficit = loser_material - winner_material
            else:
                return []  # Not down material, skip
        else:
            return []  # Can't get FEN, skip
        
        # Store lightweight reference (defer FEN/ELO extraction until get_final_results)
        # Use half-move count directly (len(moves) is already half-move count)
        final_move_number = len(moves)
        moves_back = 6
        target_half_move = max(0, final_move_number - moves_back)
        moves_to_mate = moves[target_half_move:] if target_half_move < len(moves) else moves
        
        # Check if it's a capture promotion
        is_capture = 'x' in last_move
        
        ref = {
            "game_data": self.game_data,
            "final_move_number": final_move_number,
            "target_half_move": target_half_move,
            "moves_to_mate": moves_to_mate,
            "last_move": last_move,
            "promoted_piece": promoted_piece,
            "promoted_value": promoted_value,
            "is_capture": is_capture,
            "user_is_white": self.user_is_white,
            "winner_material": winner_material,
            "loser_material": loser_material
        }
        
        self.all_findings.append(ref)
        
        return []  # Return empty - we'll build full finding in get_final_results()
    
    def _get_promoted_piece(self, move: str) -> str:
        """Extract the promoted piece from a move like 'e8=Q#' or 'exd8=N#'."""
        if '=' in move:
            idx = move.index('=')
            piece_char = move[idx + 1]
            piece_map = {'Q': 'Queen', 'R': 'Rook', 'B': 'Bishop', 'N': 'Knight'}
            return piece_map.get(piece_char, piece_char)
        return 'Unknown'
    
    def get_game_points(self, config: dict) -> int:
        """Return points for current game based on existing findings."""
        if self.found_promotion_mate:
            return config.get("promotion_mate", 45)
        return 0
    
    def _get_fen_at_move(self, pgn: str, half_move_number: int) -> Optional[str]:
        """
        Get FEN at a specific half-move number.
        
        Args:
            pgn: PGN string
            half_move_number: 0-indexed half-move number (0 = starting position, 1 = after first move, etc.)
        """
        try:
            import chess.pgn
            from io import StringIO
            
            pgn_io = StringIO(pgn)
            game = chess.pgn.read_game(pgn_io)
            if not game:
                return None
            
            board = game.board()
            
            # half_move_number is 0-indexed (0 = starting position, 1 = after first move, etc.)
            for i, node in enumerate(game.mainline()):
                if i >= half_move_number:
                    break
                board.push(node.move)
            
            return board.fen()
        except Exception:
            return None
    
    def _extract_elo(self, pgn: str, elo_header: str) -> Optional[int]:
        """Extract ELO rating from PGN header."""
        match = re.search(rf'\[{elo_header}\s+"(\d+)"\]', pgn)
        if match:
            try:
                return int(match.group(1))
            except ValueError:
                return None
        return None
    
    def _extract_fen(self, pgn: str) -> Optional[str]:
        """Extract FEN from PGN headers (CurrentPosition or FEN)."""
        # Look for FEN in headers
        match = re.search(r'\[FEN\s+"([^"]+)"\]', pgn)
        if match:
            return match.group(1)
        # Chess.com includes CurrentPosition for final position
        match = re.search(r'\[CurrentPosition\s+"([^"]+)"\]', pgn)
        if match:
            return match.group(1)
        return None
    
    def _material_from_fen(self, fen: str) -> tuple:
        """Calculate material for white and black from FEN string."""
        # FEN piece placement is first part before space
        piece_placement = fen.split()[0]
        
        white_material = 0
        black_material = 0
        
        for char in piece_placement:
            lower = char.lower()
            if lower in self.PIECE_VALUES:
                value = self.PIECE_VALUES[lower]
                if char.isupper():
                    white_material += value
                else:
                    black_material += value
        
        return white_material, black_material
    
    def get_matched_game_links(self) -> List[str]:
        """Fast path: return just the game links that matched."""
        return [ref["game_data"].metadata.link for ref in self.all_findings 
                if ref.get("game_data") and ref["game_data"].metadata.link]

    def get_final_results(self) -> List[Dict[str, Any]]:
        """
        Get final results after processing all games.
        Returns the earliest under-promotion mate (not queen) where the winner had less material.
        If no under-promotions found, returns the earliest queen promotion mate.
        NOW extracts FEN and checks material disadvantage.
        
        Returns:
            List containing a single promotion mate finding (earliest under-promotion, or queen if none)
        """
        if not self.all_findings:
            return []
        
        # Filter for under-promotions (not queen) and material disadvantage
        under_promotion_findings = []
        queen_promotion_findings = []
        
        for ref in self.all_findings:
            # Extract FEN from PGN (deferred until here)
            fen = self._extract_fen(ref["game_data"].pgn)
            if not fen:
                continue  # Skip if we can't get FEN
            
            # Calculate material from FEN
            white_material, black_material = self._material_from_fen(fen)
            
            # Subtract promoted piece (it's in the FEN but we want "before" material)
            promoted_value = ref["promoted_value"]
            
            if ref["user_is_white"]:
                winner_material = white_material - promoted_value
                loser_material = black_material
            else:
                winner_material = black_material - promoted_value
                loser_material = white_material
            
            # Material already checked in finish_game(), so this should always be true
            # But double-check for safety
            if winner_material < loser_material:
                # Check if it's an under-promotion (not queen)
                promoted_piece = ref.get("promoted_piece", "").upper()
                if promoted_piece in ['Q', 'QUEEN']:
                    queen_promotion_findings.append(ref)
                else:
                    under_promotion_findings.append(ref)
        
        # Prefer under-promotions, but fall back to queen promotions if none found
        if under_promotion_findings:
            findings_to_use = under_promotion_findings
        elif queen_promotion_findings:
            findings_to_use = queen_promotion_findings
        else:
            return []
        
        # Sort by move number (earliest first) and select the first one
        findings_to_use.sort(key=lambda x: x["final_move_number"])
        best_ref = findings_to_use[0]
        
        # Calculate material for the selected finding
        fen = self._extract_fen(best_ref["game_data"].pgn)
        white_material, black_material = self._material_from_fen(fen)
        promoted_value = best_ref["promoted_value"]
        
        if best_ref["user_is_white"]:
            winner_material = white_material - promoted_value
            loser_material = black_material
        else:
            winner_material = black_material - promoted_value
            loser_material = white_material
        
        # Extract ELO for the selected finding
        white_elo = self._extract_elo(best_ref["game_data"].pgn, "WhiteElo")
        black_elo = self._extract_elo(best_ref["game_data"].pgn, "BlackElo")
        
        # Build replay data structure for frontend (uses config file)
        from ..replay_helper import build_replay_data
        replay_data = build_replay_data(
            game_data=best_ref["game_data"],
            key_half_move=best_ref["final_move_number"],
            feature_name="promotion_mate"  # Loads settings from replay_config.json
        )
        
        # Build full finding with extracted data
        finding = {
            "feature_name": "promotion_mate",
            "display_name": "Promotion Mate",
            "game_metadata": {
                "white": best_ref["game_data"].metadata.white,
                "black": best_ref["game_data"].metadata.black,
                "link": best_ref["game_data"].metadata.link,
                "white_elo": white_elo,  # Extracted here, not during processing!
                "black_elo": black_elo,  # Extracted here, not during processing!
                "user_color": "white" if best_ref["user_is_white"] else "black",
                # New simplified format for frontend navigation
                "all_moves": replay_data["all_moves"],
                "key_position_index": replay_data["key_position_index"],
                "fen": replay_data["fen"],
                # Include full PGN - contains all clock annotations/timestamps
                "pgn": best_ref["game_data"].pgn
            },
            "position_link": f"{best_ref['game_data'].metadata.link}?move={best_ref['final_move_number']}" if best_ref["game_data"].metadata.link else None,
            "result_data": {
                "promoted_piece": {
                    "value": best_ref["promoted_piece"],
                    "label": "Promoted To"
                },
                "material_difference": {
                    "value": loser_material - winner_material,
                    "label": "Material Deficit"
                }
            }
        }
        
        return [finding]

