...
import { tagDisplayName, isVisibleOnGamesPage } from '../utils/tagDisplay';
...
export default function GamesPage() {
  ...
  const sortedTags = Array.from(allTags.entries())
    .filter(([tag]) => isVisibleOnGamesPage(tag))
              ...
              >
                <div className="flex items-start gap-5">
                  ...
                  <div className="flex-1 min-w-0 py-1">
                    ...
                    {/* Tags */}
                    {game.tags.filter(isVisibleOnGamesPage).length > 0 && (
                      <div className="flex flex-wrap gap-2">
                        {game.tags.filter(isVisibleOnGamesPage).map(tag => (
                          <span
                            key={tag}
