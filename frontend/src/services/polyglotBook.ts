 ...
 *
 * Parses a Polyglot .bin file and checks whether a given move
 * (identified by FEN + UCI move) exists in the book.
 *
 * Format: sorted array of 16-byte entries:
 *   bytes 0-7:  Zobrist hash (big-endian u64)
 *   bytes 8-9:  encoded move (big-endian u16)
 *   bytes 10-11: weight (big-endian u16)
 *   bytes 12-15: learn data (unused)
 */

// ---------------------------------------------------------------------------
// Polyglot Zobrist random table (781 entries)
// Source: the canonical table used by PolyGlot / python-chess / Arena etc.
// Layout: 768 piece-square keys + 4 castling keys + 8 en-passant keys + 1 turn key
// ---------------------------------------------------------------------------

/* eslint-disable */
const RANDOM64: bigint[] = [
  // piece-square: 12 pieces × 64 squares = 768 entries
  // piece order (polyglot): bP wP bN wN bB wB bR wR bQ wQ bK wK
  // index = kind * 64 + square  (square = rank*8 + file, rank 0 = rank 1)
...
// Piece-square table offsets in the RANDOM64 array.
// Polyglot piece kinds: 0=bP 1=wP 2=bN 3=wN 4=bB 5=wB 6=bR 7=wR 8=bQ 9=wQ 10=bK 11=wK
// Index into RANDOM64: kind * 64 + square  (square = rank*8 + file, rank 0 = rank 1)
const PIECE_KIND: Record<string, number> = {
  'p': 0,  'P': 1,
  'n': 2,  'N': 3,
  'b': 4,  'B': 5,
  'r': 6,  'R': 7,
...
const CASTLING_KEY_INDEX = 768; // 768=K, 769=Q, 770=k, 771=q
const EP_KEY_INDEX = 772;      // 772..779 = files a..h
const TURN_KEY_INDEX = 780;
...
/**
 * Fetch and cache the Polyglot opening book.
 * Safe to call multiple times — subsequent calls are no-ops.
 */
export async function loadBook(): Promise<void> {
  if (bookData) return;
  try {
    const resp = await fetch('/books/gm2001.bin');
    if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
    const buf = await resp.arrayBuffer();
    bookData = new DataView(buf);
  } catch {
    // Graceful degradation — analysis proceeds without book labels.
    bookData = null;
  }
}

/**
 * Check whether a move exists in the book for the given position.
 *
 * @param fen  - FEN of the position *before* the move
 * @param uci  - UCI move string (e.g. "e2e4", "e1g1", "e7e8q")
 */
export function isBookMove(fen: string, uciMove: string): boolean {
  if (!bookData) return false;
  const hash = polyglotHash(fen);
  const entries = findEntries(hash);
  for (const entry of entries) {
    if (polyglotMoveToUci(entry.move) === uciMove) return true;
  }
  return false;
}
...
function polyglotHash(fen: string): bigint {
  ...
  for (let rank = 7; rank >= 0; rank--) {
    const row = ranks[7 - rank]; // FEN starts from rank 8
    let file = 0;
    for (const ch of row) {
      if (ch >= '1' && ch <= '8') {
        file += parseInt(ch);
        ...
        if (kind !== undefined) {
          ...
          hash = (hash ^ RANDOM64[kind * 64 + sq]) & MASK64;
        }
        file++;
      }
    }
  ...
  // Turn — white to move gets the turn key
  if (turn === 'w') {
    hash = (hash ^ RANDOM64[TURN_KEY_INDEX]) & MASK64;
  }

  // Castling
  if (castling !== '-') {
    if (castling.includes('K')) hash = (hash ^ RANDOM64[CASTLING_KEY_INDEX]) & MASK64;
    if (castling.includes('Q')) hash = (hash ^ RANDOM64[CASTLING_KEY_INDEX + 1]) & MASK64;
    if (castling.includes('k')) hash = (hash ^ RANDOM64[CASTLING_KEY_INDEX + 2]) & MASK64;
    if (castling.includes('q')) hash = (hash ^ RANDOM64[CASTLING_KEY_INDEX + 3]) & MASK64;
  }

  // En passant — only hash if a pawn of the side to move can actually capture
  if (ep !== '-') {
    const epFile = ep.charCodeAt(0) - 'a'.charCodeAt(0);
    const epRank = parseInt(ep[1]) - 1; // 0-indexed rank

    if (canCaptureEp(placement, turn, epFile, epRank)) {
      hash = (hash ^ RANDOM64[EP_KEY_INDEX + epFile]) & MASK64;
    }
  }
...
/**
 * Check if a pawn of the side to move is adjacent to the EP target
 * and could capture en passant.
 */
function canCaptureEp(placement: string, turn: string, epFile: number, epRank: number): boolean {
  // The capturing pawn must be on the rank just behind the EP square
  const pawnRank = turn === 'w' ? epRank - 1 : epRank + 1;
  const pawnChar = turn === 'w' ? 'P' : 'p';

  // Check left and right of the ep file for a pawn
  const ranks = placement.split('/');
  const row = ranks[7 - pawnRank];
  if (!row) return false;

  // Expand the row to an 8-char string where each char is the piece or '.'
  let expanded = '';
  for (const ch of row) {
    if (ch >= '1' && ch <= '8') {
  ...
  // Check adjacent files for a pawn of the correct color
  if (epFile > 0 && expanded[epFile - 1] === pawnChar) return true;
  if (epFile < 7 && expanded[epFile + 1] === pawnChar) return true;

  return false;
}

// ---------------------------------------------------------------------------
// Binary search over sorted 16-byte entries
// ---------------------------------------------------------------------------

interface BookEntry {
  move: number;   // 16-bit encoded move
  weight: number;
}

function findEntries(hash: bigint): BookEntry[] {
  if (!bookData) return [];

  const entryCount = bookData.byteLength / 16;
  ...
  // Collect all entries with this hash (they're contiguous)
  for (let i = found; i < entryCount; i++) {
    if (readHash(i) !== hash) break;
    const offset = i * 16;
    results.push({
      move: bookData.getUint16(offset + 8),
      weight: bookData.getUint16(offset + 10),
    });
  }
...
function readHash(index: number): bigint {
  const offset = index * 16;
  const hi = BigInt(bookData!.getUint32(offset)) << 32n;
  const lo = BigInt(bookData!.getUint32(offset + 4));
  return hi | lo;
}

// ---------------------------------------------------------------------------
// Decode 16-bit Polyglot move encoding to UCI string
// ---------------------------------------------------------------------------

function polyglotMoveToUci(encoded: number): string {
  const toFile   = (encoded)       & 0x7;
  const toRow    = (encoded >> 3)   & 0x7;
  const fromFile = (encoded >> 6)   & 0x7;
  const fromRow  = (encoded >> 9)   & 0x7;
  const promo    = (encoded >> 12)  & 0x7;

  const files = 'abcdefgh';
  let from = files[fromFile] + (fromRow + 1);
  let to = files[toFile] + (toRow + 1);

  // Castling normalization: Polyglot encodes king→rook square,
  // but UCI uses king→destination (2 squares).
  if (from === 'e1' && to === 'h1') to = 'g1'; // white O-O
  if (from === 'e1' && to === 'a1') to = 'c1'; // white O-O-O
  if (from === 'e8' && to === 'h8') to = 'g8'; // black O-O
  if (from === 'e8' && to === 'a8') to = 'c8'; // black O-O-O
